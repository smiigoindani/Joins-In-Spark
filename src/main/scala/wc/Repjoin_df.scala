...
object ReplicatedJoinDF {
  
  def main(args: Array[String]) {
    val logger: org.apache.log4j.Logger = LogManager.getRootLogger
    if (args.length != 2) {
      logger.error("Usage:\nwc.ReplicatedJoinDF<input dir> <output dir>")
      System.exit(1)
    }
    val conf = new SparkConf().setAppName("ReplicatedJoinDF")
...
//    try { hdfs.delete(new org.apache.hadoop.fs.Path(args(1)), true) } catch { case _: Throwable => {} }
    ...
    val pathLength2 = XY_df.join(broadcast(YZ_df)).where($"xtoy.user_Y1" === $"ytoz.user_Y2" && !($"xtoy.user_X" === $"ytoz.user_Z1"))

    val triangle = pathLength2.as("path").join(broadcast(ZX_df)).where($"path.user_Z1"===$"ztox.user_Z2" && $"path.user_X" === $"ztox.user_Z")
    ...
    //println("TriangleCount: "+ triangle.count()/3)
