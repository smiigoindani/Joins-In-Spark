...
app.name=ReplicatedJoinDF 
jar.name=spark-demo.jar
maven.jar.name=spark-demo-1.0.jar
job.name=rep.ReplicatedJoinDF 
local.master=local[4]
local.input=input
local.output=output

# Pseudo-Cluster Execution
hdfs.user.name=simrangoindani
hdfs.input=input
hdfs.output=output
# AWS EMR Execution
aws.emr.release=emr-5.17.0
aws.bucket.name=sparkrepjoin
aws.input=input
aws.output=output
aws.log.dir=log
aws.num.nodes=4
...
local: jar clean-local-output
	spark-submit --class ${job.name} --master ${local.master} --driver-memory 5g --name "${app.name}" ${jar.name} ${local.input} ${local.output}
